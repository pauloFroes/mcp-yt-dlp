...
import { z } from "zod";
import { execCommand, toolResult, toolError, isUrl, createTempDir, cleanupTempDir } from "../runner.js";
  ...
  server.registerTool(
    ...
    async ({ url }) => {
      try {
        ...
        return toolError(`Failed to get video info: ${(error as Error).message}`);
      }
    },
  );
  ...
  server.registerTool(
    ...
    async ({ url, output_dir, filename }) => {
      ...
      try {
        ...
        return toolError(`Failed to download video: ${(error as Error).message}`);
      }
    },
  );
  ...
  server.registerTool(
    ...
    async ({ url, output_dir, filename }) => {
      ...
      try {
        ...
        if (tempDir) await cleanupTempDir(tempDir);
        return toolError(`Failed to download audio: ${(error as Error).message}`);
      }
    },
  );
}
